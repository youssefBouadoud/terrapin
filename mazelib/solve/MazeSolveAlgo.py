...
class MazeSolveAlgo(metaclass=abc.ABCMeta):
    def solve(self, grid, start, end):
        """helper method to solve an init the solver before solving the maze
    ...
    def _solve_preprocessor(self, grid, start, end):
        ...
        assert (
                0 <= start[0] < grid.shape[0]
        ), "Entrance is outside the grid."
        assert (
                0 <= start[1] < grid.shape[1]
        ), "Entrance is outside the grid."
        assert 0 <= end[0] < grid.shape[0], "Entrance is outside the grid."
        assert 0 <= end[1] < grid.shape[1], "Entrance is outside the grid."
    ...
    def _midpoint(self, a, b):
        ...
        Args:
            ...
            b (tuple): second cell
        Returns:
            tuple: cell halfway between the first two
        """
    ...
    def _push_edge(self, cell):
        """You may need to find the cell directly inside a start or end cell.
